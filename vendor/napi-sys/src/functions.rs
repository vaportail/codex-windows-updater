...
pub(super) unsafe fn load_all() -> Result<libloading::Library, libloading::Error> {
  #[cfg(windows)]
  let host = {
    let executable: libloading::Library = libloading::os::windows::Library::this()?.into();
    if executable.get::<unsafe extern "C" fn()>(b"napi_create_function\0").is_ok() {
      executable
    } else {
      // Owl exports Node-API from chrome.dll instead of the executable.
      // Acquire an already loaded module only; never search/load another DLL.
      libloading::os::windows::Library::open_already_loaded("chrome.dll")?.into()
    }
  };
